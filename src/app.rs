...
fn file_type_style(ft: &str) -> (&'static str, egui::Color32) {
    match ft {
        "pdf" => ("\u{1F4C4}", egui::Color32::from_rgb(200, 50, 50)),
        "xlsx" | "xls" => ("\u{1F4CA}", egui::Color32::from_rgb(30, 140, 60)),
        "docx" => ("\u{1F4DD}", egui::Color32::from_rgb(40, 80, 160)),
        "pptx" => ("\u{1F4F9}", egui::Color32::from_rgb(200, 100, 30)),
        _ => ("\u{1F4C4}", egui::Color32::GRAY),
    }
}
...
fn highlight_text(text: &str, query: &str) -> String {
    ...
    for (start, _) in lower_text.match_indices(&lower_query) {
        ...
        result.push_str("\u{1F7E2}"); // green square marker
        result.push_str(&text[start..start + query.len()]);
        result.push_str("\u{1F7E2}");
...
const DAY_NAMES_ES: [&str; 7] = ["Lu", "Ma", "Mi", "Ju", "Vi", "Sa", "Do"];
const DAY_NAMES_EN: [&str; 7] = ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"];
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button(tr(l, "Cambiar", "Change")).clicked() {
                            ...
                            if let Some(new_path) = file {
                                let stem = new_path.file_stem().unwrap_or_default().to_string_lossy().to_string();
